...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if '/' in seq.name:
        save_seq_name = seq.name.split('/')[-1]
        base_results_path = os.path.join(tracker.results_dir, save_seq_name)
        segmentation_path = os.path.join(tracker.segmentation_dir, save_seq_name)
    else:
