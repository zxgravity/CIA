...
class DiMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        ################ visualization ################
        # score_map = score_map / (score_map**0.5).max()
        score_map = (score_map - score_map.min()) / (score_map.max() - score_map.min())
        save_start = 1
        save_end = -1
        if self.frame_num >= save_start and self.frame_num < save_end:
            im_crop_org = np.uint8(np.array(im_patches[0].permute(1, 2, 0).cpu()))
            score_map_add = F.upsample_bilinear(score_map[None,None,:,:], size=im_patches[0].shape[-1])
            score_map_add = np.array(score_map_add[0][0].cpu())
