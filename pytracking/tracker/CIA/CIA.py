...
from ltr.models.utils import *

import cv2 
import numpy as np 


# with open('/media/syj/9b737a42-dfb1-4eff-8c7b-cdf60ce9feb0/pzx/code/tracking/pyt_cls/pytracking/upper_cls_map.txt', 'r') as f:
with open('upper_cls_map.txt', 'r') as f:
    upper_cls_map = [x.strip() for x in f.readlines()]


class CIA(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        self.prroi_pool = PrRoIPool2D(1, 1, 1/16.)
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # Refine classification features
        # with torch.no_grad():
        #     test_x = self.net.upper_classifier.semantic_trans(test_x, training=False)
        ...
        upper_feat = backbone_feat['layer4']
        box_center = (self.pos - sample_pos[scale_ind,:]) / sample_scales[scale_ind] + (self.img_sample_sz - 1) / 2
        box_sz = self.target_sz / sample_scales[scale_ind]
        target_ul = box_center - (box_sz - 1) / 2
        upper_box = torch.cat([target_ul.flip((0,)), box_sz.flip((0,))])[None, :].to(upper_feat.device)
        if self.params.get('use_upper_classifier', False):
            with torch.no_grad():
                upper_cls_pred = self.net.upper_classifier.classify(upper_feat, upper_box)
                upper_cls_pred = F.softmax(upper_cls_pred)
            upper_cls_ind = torch.argmax(upper_cls_pred, dim=-1)
            upper_cls_score = upper_cls_pred[upper_cls_ind]
            upper_cls = upper_cls_map[upper_cls_ind]
        ...
        if self.params.get('use_upper_classifier', False):
            self.debug_info['upper_cls' + self.id_str] = '{}({})'.format(upper_cls, upper_cls_score)
        if self.visdom is not None:
            self.visdom.register(score_map, 'heatmap', 2, 'Score Map' + self.id_str)
            self.visdom.register(self.debug_info, 'info_dict', 1, 'Status')
            try:
                ins_score_map = corr(self.init_ins_proj[0:1], backbone_feat['ins_proj'])
                self.visdom.register(ins_score_map, 'heatmap', 2, 'Ins Score Map')
            except:
                pass 
    ...
    def get_classification_features(self, backbone_feat):
        # Refine classification features
        with torch.no_grad():
            x = self.net.get_backbone_clf_feat(backbone_feat)
            if self.net.refine_flag:
                x = self.net.upper_classifier.semantic_trans(x, training=False)
            x = self.net.classifier.extract_classification_feat(x)
            return x
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        rois = bb2roi(target_boxes)
        try:
            self.init_ins_proj = self.prroi_pool(init_backbone_feat['ins_proj'], rois[0:-2])
        except:
            pass 
