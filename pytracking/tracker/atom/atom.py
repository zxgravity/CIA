...
class ATOM(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # if self.frame_num >= 185:
        #     pdb.set_trace()

        # Get sample
        sample_pos = self.pos.round()
        sample_scales = self.target_scale * self.params.scale_factors
        test_x, im_crop_org = self.extract_processed_sample(im, self.pos, sample_scales, self.img_sample_sz)
        ...
        save_start = 1
        save_end = -1
        if self.frame_num >= save_start and self.frame_num < save_end:
            im_crop_org = np.uint8(np.array(im_crop_org.permute(1, 2, 0).cpu()))
            score_map_add = np.array(score_map[0][0].cpu())
            score_map_add[score_map_add < 0] = 0 
            score_map_add[score_map_add > 1] = 1 
            heat_map_add = cv2.applyColorMap(np.uint8(score_map_add * 255), cv2.COLORMAP_JET)
            # heat_map_add = cv2.cvtColor(heat_map_add, cv2.COLOR_BGR2RGB)
            mix_map = cv2.addWeighted(im_crop_org, 0.5, heat_map_add, 0.5, 0)
            cv2.imwrite('mix_map/{}.jpg'.format(self.frame_num), mix_map)
        if self.frame_num == save_end:
            pdb.set_trace()
        ###############################################
    ...
    def extract_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        feature_map, patch_coords, im_patches =  self.params.features.extract(im, pos, scales, sz, return_patches=True)
        return feature_map, im_patches[0] 
    ...
    def extract_processed_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor) -> (TensorList, TensorList):
        x, im_crop_org = self.extract_sample(im, pos, scales, sz)
        return self.preprocess_sample(self.project_sample(x)), im_crop_org 
