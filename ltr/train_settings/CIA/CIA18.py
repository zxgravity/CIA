...
def run(settings):
    settings.description = 'Default train settings for CIA with ResNet18 as backbone.'
    ...
    dataset_train = sampler.DiMPSampler([lasot_train, got10k_train, trackingnet_train, coco_train], [0.25,1,1,1],
                                        samples_per_epoch=128000, max_gap=30, num_test_frames=3, num_train_frames=3,
    ...
    net = dimpnet.cia18(filter_size=settings.target_filter_sz, backbone_pretrained=True, optim_iter=5,
    ...
    loss_weight = {'iou': 1, 'upper_clf':0.1, 'test_clf': 100, 
    ...
    actor = actors.CIDiMPActor(net=net, objective=objective, loss_weight=loss_weight, upper_cls_map=upper_cls_map)
