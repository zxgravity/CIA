...
def run(settings):
    settings.description = 'Default train settings for CIA with ResNet50 as backbone.'
    ...
    dataset_train = sampler.DiMPSampler([lasot_train, got10k_train, trackingnet_train, coco_train], [1, 1, 1, 1],
                                        samples_per_epoch=200, max_gap=200, num_test_frames=3, num_train_frames=3,
    ...
    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size,
                           num_workers=settings.num_workers,
                           shuffle=False, drop_last=True, epoch_interval=20, stack_dim=1)

    # Create network and actor
    net = dimpnet.cia50(filter_size=settings.target_filter_sz, backbone_pretrained=True, optim_iter=5,
    ...
    objective = {'bb_ce': klreg_losses.KLRegression(), 
                 'upper_clf':ltr_losses.UpperLoss(crt=nn.CrossEntropyLoss(reduce=False)), 
                 'test_clf': ltr_losses.LBHinge(threshold=settings.hinge_threshold),
                 'contrastive': ltr_losses.InfoNCELoss()}

    loss_weight = {'bb_ce': 0.01, 'upper_clf':0.1, 'test_clf': 100, 
                   'test_init_clf': 100, 'test_iter_clf': 400,
                   'contrastive': 1}

    upper_cls_list = lasot_train.class_list 
    upper_cls_list.sort()
    upper_cls_map = {}
    for i in range(len(upper_cls_list)):
        upper_cls_map[upper_cls_list[i]] = i

    actor = tracking_actors.CIKLDiMPActor(net=net, objective=objective, loss_weight=loss_weight, upper_cls_map=upper_cls_map)
    ...
    optimizer = optim.Adam([{'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 5e-5},
                            ...
                            {'params': actor.net.upper_classifier.parameters()},
