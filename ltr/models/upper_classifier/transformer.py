...
class TransformerEncoder_v2(nn.Module):
    ...
    def forward(self, q, k, v,
        ...
        if return_intermediate:
            ...
            for layer in self.layers:
                output = layer(output, k, v, src_mask=mask,
                               src_key_padding_mask=src_key_padding_mask, pos=pos,
                               training=training)
                if self.norm is None:
                    output_list.append(output)
...
class TransformerEncoderLayer_v2(nn.Module):
    ...
    def forward_post(self,
                     ...
                     pos: Optional[Tensor] = None,
                     training=True):
        q_ = self.with_pos_embed(q, pos)  # add pos to q
        if self.divide_norm:
            # print("encoder divide by norm")
            q_ = q_ / torch.norm(q_, dim=-1, keepdim=True) * self.scale_factor
            k = k / torch.norm(k, dim=-1, keepdim=True)
        src2 = self.self_attn(q_, k, v, attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask)[0]
        if training:
    ...
    def forward_pre(self,
        ...
        q_ = self.norm1(q)
        k = self.norm1(k)
        q_ = self.with_pos_embed(q_, pos)
        src2 = self.self_attn(q_, k, value=v, attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask)[0]
        if training:
            src = q + self.dropout1(src2)
        else:
    ...
    def forward(self, 
                ...
                training=True):
        if self.normalize_before:
            return self.forward_pre(q, k, v, src_mask, src_key_padding_mask, pos, training=training)
        return self.forward_post(q, k, v, src_mask, src_key_padding_mask, pos, training=training)
